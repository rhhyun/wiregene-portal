...
`portal.wiregene.com`에서 `wiregene`과 `rhhyun`이 모두 Basic Auth를 통과하지 못하면, 현재 접속 경로가 Synology인지 Vercel인지 먼저 확인합니다.
...
아래 명령은 `rhhyun`과 `wiregene`을 같은 임시 비밀번호로 Vercel production `APP_BASIC_AUTH_USERS`에 설정하고, 둘 다 관리자 목록에 넣은 뒤 production redeploy와 로그인 확인까지 수행합니다.
